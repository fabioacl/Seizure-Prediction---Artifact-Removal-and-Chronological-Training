...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan  9 14:33:29 2022
...
for patient_index in range(1,2):
    ...
    if os.path.exists(f'{model_type} Chronology/Patient {patient_number}')==False:
        os.mkdir(f'{model_type} Chronology/Patient {patient_number}')
    ...
    for run_index in range(0,31):
        ...
        for train_index in range(num_testing_seizures):
            ...
            np.save(f'{model_type} Chronology/Patient {patient_number}/standardisation_values_{patient_number}_{run_index}_{test_index}',norm_values)
            # Get mini-batch size
            batch_size = 64
            ...
            model_checkpoint_cb = ModelCheckpoint(f'{model_type} Chronology/Patient {patient_number}/seizure_prediction_model_{patient_number}_{run_index}_{test_index}.h5', 'val_loss', save_best_only=True, verbose=1, mode='min')
            ...
            # Load best weights
            model.load_weights(f'{model_type} Chronology/Patient {patient_number}/seizure_prediction_model_{patient_number}_{run_index}_{test_index}.h5')
        ...
        with open(f'{model_type} Chronology/Patient {patient_number}/all_results_{patient_number}_{model_type}_{run_index}.pkl','wb') as file:
            pickle.dump(all_results,file)
        save_results(patient_number,f'{model_type} Chronology/results_{patient_number}_{model_type}.csv',all_sensitivities,all_fpr_h,sop,test_seizures,avg_last_epoch)
        # Clear variables
