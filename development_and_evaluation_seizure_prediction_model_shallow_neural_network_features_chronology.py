...
# -*- coding: utf-8 -*-
"""
Created on Thu May  5 17:14:13 2022
...
from ComputeEEGFeatures import ComputeEEGFeatures
...
def save_results(patient_number,filename,all_sensitivities,all_fpr_h,sop,tested_seizures,ss_rand_pred,ss_surrogate,fpr_h_surrogate,last_epoch):
...
for patient_index in range(0,41):
    ...
    num_seizures = len(dataset)
    num_training_seizures = round(training_ratio * num_seizures)
    num_testing_seizures = num_seizures - num_training_seizures
    
    if os.path.exists(f'Only Features Chronology/Patient {patient_number}')==False:
        os.mkdir(f'Only Features Chronology/Patient {patient_number}')
    
    for run_index in range(0,31):
        print(f'Run Index: {run_index}')
        
        # Initialise arrays of results
        all_sensitivities = []
        all_surrogate_sensitivities = []
        all_fpr_h = []
        all_surrogate_fpr_h = []
        all_fp_values = []
        all_alarms = []
        ...
        for train_index in range(num_testing_seizures):
            
            # The number of training seizures increases over time
            training_seizures = num_training_seizures + train_index
            
            #Correct test index. It is the first seizures after the last training seizures.
            test_index = training_seizures 
            ...
            training_features_data = ComputeEEGFeatures(training_data).calculate_window_features(feature_groups)
            
            num_windows = training_features_data.shape[0]
            training_features_data = training_features_data.reshape((num_windows,-1))
            
            # Divide the training data into training and validation sets
            validation_ratio = 0.2
            training_indexes = np.arange(num_windows)
            X_train,X_val,train_indexes,val_indexes = train_test_split(training_data,training_indexes,
                                                                        test_size=validation_ratio,random_state=run_index,
                                                                        stratify=training_labels)
            # Raw Sets
            y_train = training_labels_categorical[train_indexes]
            y_val = training_labels_categorical[val_indexes]
            
            # Feature Sets
            X_feat_train = training_features_data[train_indexes]
            X_feat_val = training_features_data[val_indexes]
            
            #------------Train Seizure Prediction Model-----------
            
            print("Train Seizure Prediction Model...")
            # Get standardisation values
            norm_values = [np.mean(X_feat_train,axis=0),np.std(X_feat_train,axis=0)]
            np.save(f'Only Features Chronology/Patient {patient_number}/standardisation_values_{patient_number}_{run_index}_{test_index}',norm_values)
            # Get mini-batch size
            batch_size = 64
            X_train = X_feat_train
            X_val = X_feat_val
            ...
            model_checkpoint_cb = ModelCheckpoint(f'Only Features Chronology/Patient {patient_number}/seizure_prediction_model_{patient_number}_{run_index}_{test_index}.h5', 'val_loss', save_best_only=True, verbose=1, mode='min')
            ...
            # Load best weights
            model.load_weights(f'Only Features Chronology/Patient {patient_number}/seizure_prediction_model_{patient_number}_{run_index}_{test_index}.h5')
        
            # Predict labels
            X_test_raw = dataset[test_index]
            X_test_feat = ComputeEEGFeatures(X_test_raw).calculate_window_features(feature_groups)
            num_test_windows = X_test_feat.shape[0]
            X_test_feat = X_test_feat.reshape((num_test_windows,-1))
            X_test_feat = (X_test_feat-norm_values[0])/norm_values[1]
            X_test = X_test_feat
            ...
            all_last_epochs.append(last_epoch)
        ...
        ss_surrogate = None
        ss_fpr_h = None
        ss_rand_pred = f_RandPredictd(test_seizures,avg_fpr_h,1,sop/60,0.05)
        ...
        all_results = {'Sensitivities':all_sensitivities,
                        ...
                        'Last Epoch':avg_last_epoch,
                        'Seizure Onset Datetimes':all_seizure_onset_datetimes}
        
        with open(f'Only Features Chronology/Patient {patient_number}/all_results_{patient_number}_{model_type}_{run_index}.pkl','wb') as file:
            pickle.dump(all_results,file)
        save_results(patient_number,f'Only Features Chronology/results_{patient_number}_{model_type}.csv',all_sensitivities,all_fpr_h,sop,test_seizures,ss_rand_pred,ss_surrogate,ss_fpr_h,avg_last_epoch)
        # Clear variables
